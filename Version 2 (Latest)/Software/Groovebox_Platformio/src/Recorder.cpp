...
*Radio Module Config 
*/
const byte radioReset = 14;
const byte radioSdio = 18;
const byte radioSclk = 19;
...
void Recorder::menuCheck() {
  ...
  switch (screen) {
    case 0://list files
      ...
      if (eraseButton and !mainSelect and !eraseButtonPrev){
        fileErasePrompt = 1;
        draw = 1;
      }
      else if (eraseButton and mainSelect and !selectButtonPrev){
        fileErasePrompt = 0;
        currentDirectory = SD.open("/Recs");
        currentDirectory.rewindDirectory();
        File entry =  currentDirectory.openNextFile();
    ...
    case 1://rec screen
      ...
      if (mainSelect and !selectButtonPrev and recordStatus==3){//erase confirm
        recordStatus = 0;
        oled.drawRoundRect(29,63,19,19,5,BLACK);
        oled.fillRoundRect(53,63,19,19,5,BLACK);
        oled.drawRoundRect(77,63,19,19,5,BLACK);
      ...
      else if (mainSelect and !selectButtonPrev and recordStatus==4){
        recordStatus = 0;
        oled.drawRoundRect(29,63,19,19,5,BLACK);
        oled.fillRoundRect(53,63,19,19,5,BLACK);
        oled.drawRoundRect(77,63,19,19,5,BLACK);
...
void Recorder::recordAndPlayback(){
  switch (screen){
    case 0://list files
...
void Recorder::startPlayingPreview() {
  recFile.close();
  switch (screen){
    case 0://list files
...
void Recorder::continuePlayingPreview() {
  if (!playSdRec.isPlaying()) {
    playSdRec.stop();
  }
}

void Recorder::stopPlayingPreview() {
  playSdRec.stop();
}